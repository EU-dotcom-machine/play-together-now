...
export default defineMcp({
  ...
  version: "0.1.0",
  instructions:
    "Tools for Esportes Unidos, a sports meetup app. Use `list_sports` to discover sports, `search_upcoming_games` to find public games (optionally filtered by sport), `get_game` for full details of one game, and `my_agenda` for the signed-in user's own upcoming games.",
  auth: auth.oauth.issuer({
    issuer: `https://${projectRef}.supabase.co/auth/v1`,
    acceptedAudiences: "authenticated",
  }),
  tools: [listSports, searchUpcomingGames, getGame, myAgenda],
});
